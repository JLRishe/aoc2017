const { __, compose, when, curry, map, add, ifElse, gte, adjust, lt, length, both, prop } = require('ramda');
...
// [*] -> Number -> Boolean
const outOfBounds = curry((arr, i) => i < 0 || i >= length(arr));

// (Number -> Number) -> [String] -> Number
const countJumps = jumpAdjust => instructs => countCycles(
    nextWorld(jumpAdjust),
    compose(outOfBounds(instructs), prop('i')),
